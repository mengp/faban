 ...
 * Subclasses execute the provided driver, keeps track of response times, think
 * times, transaction counts, etc.
 *
...
public abstract class AgentThread extends Thread {
    ...
    public static enum RunState {
        ...
        INITIALIZING,
        /**
         * The {@link AgentThread} is executing {@link BenchmarkOperation}s to
         * warm the SUT.
         */
        PRE_RUN,
        /**
    ...
    /**
     * Logs the normal errors occuring in operations. The operation is marked as
     * failed.
     *
    ...
    /**
     * Executes the method market with
     *
     * @OnceBefore in thread 0.
     */
    void preRun() {
    ...
    /**
     * Executes the method marked with
     *
     * @OnceAfter in thread 0.
     */
    void postRun() {
    ...
    /**
     * This method blocks until the start time is set by the master. Called by
     * AgentThread implementations.
     */
    void waitStartTime() {
        try {
    ...
    void validateTimeCompletion(BenchmarkDefinition.Operation op) {
        ...
        // proper message.
        /* Judge if using ExtendedJSTransport. Added by limp */
        if (timingInfo.responseInfoList.isEmpty()) {
            ...
            /* check for the unusual case of the time for responseList. 
             * author limp */
     ...
     * Updates the inRamp parameter accordingly. This is only supposed to be
