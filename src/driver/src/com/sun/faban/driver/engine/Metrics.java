 ...
 * without any additional statistics, this class can be used right away. This
 * class should also be extended to collect all additional statistics.
 *
...
public class Metrics implements Serializable, Cloneable,
     ...
     */
    /**
     * Number of successful transactions during steady state. This is used for
     * final reporting and in-flight reporting of averages.
     */
    protected int[] txCntStdy;
    /**
     * Number of successful transactions total. This is used for in-flight
     * reporting only.
     */
    protected int[] txCntTotal;
    /**
     * Number of failed transactions during steady state. This is used for final
     * reporting and in-flight reporting of averages.
     */
    protected int[] errCntStdy;
    /**
     * Number of failed transactions total. This is used for in-flight reporting
     * only.
     */
    protected int[] errCntTotal;
    /**
     * Number of transactions the delay time was successfuly recorded. Note that
     * some transactions while failing may still have the delay time recorded.
     * Transactions that do not have the delay time recorded are transactions
     * that fail before the critical section.
     */
    protected int[] delayCntStdy;
    /**
     * Sum of response times during steady state. This is used for final
     * reporting and runtime reporting of averages.
     */
    ...
    /**
     * Sum of high response times that fall into the overflow bucket. This is
     * used for calculating the representative value for the overflow bucket to
     * establish a reasonable standard deviation.
     */
    ...
    /**
     * The thruput graph. This is updated throughout the run, not only in steady
     * state. The graph accumulates tx count during the run. The final results
     * need to be divided by the graph bucket size.
     */
    protected int[][] thruputGraph;	/* Thruput graph */

    /**
     * Graph of accumulated response times over the course of the run. This data
     * need to be divided by the accumulated tx count for the bucket to get the
     * avg response time in that bucket.
     */
    ...
    /**
     * The final resulting metric. This field is only populated after printing
     * the summary report
     */
    ...
    public Metrics(AgentThread agent) {
        ...
        double delayHistMax = driverConfig.operations[0].cycle.getHistogramMax();

        // Find the max delay time histogram among ops, in ns
        for (int i = 1; i < txTypes; i++) {
            double opMaxDelay = driverConfig.operations[i].cycle.getHistogramMax();
    ...
    public void recordTx() {
        ...
        if (timingInfo.responseInfoList.isEmpty()) {
            ...
            responseTime = endTimeNanos - timingInfo.invokeTime
                    - timingInfo.pauseTime;
        } else {
            int size = timingInfo.responseInfoList.size();
            endTimeNanos = timingInfo.responseInfoList.get(size - 1).endTime;
            for (DriverContext.ResponseInfo info : timingInfo.responseInfoList) {
                responseTime += info.endTime - info.startTime;
            }
        }
    ...
    public void recordError() {
        ...
        if (thread.driverContext.timingInfo.responseInfoList.isEmpty()) {
    ...
    public void recordDelayTime() {
        ...
        CycleType cycleType = RunInfo.getInstance().driverConfig.operations[thread.currentOperation].cycle.cycleType;
        switch (cycleType) {
            case CYCLETIME:
    ...
    /**
     * Wraps up the metric for serialization/transportation and/or further
     * processing.
     */
    public void wrap() {
    ...
    /**
     * This method aggregates the stats with the stats of another thread. It is
     * called repeatedly, and the called passes it the stats of a different
     * thread, each time
     *
    ...
    /**
     * Aggregates 2 sum of squares S1 and S2, each of these variances come from
     * a sample set with N1 and N2 samples and sum of T1 and T2 respectively.
     * This aggregation uses Chan's algorithm published in <a href=
    ...
    /**
     * Subtracts 2 sum of squares S and S1 to obtain S2. This will allow us to
     * find the variance or standard deviation for response times for a time
     * slot. This method is used for calculating the current standard deviation
     * in the runtime stats. The results are estimates and may not be totally
     * accurate. In most cases it is pretty accurate, though. We do our best to
    ...
    /**
     * Prints the summary report for the statistics. This will usually be called
     * once the statistics have been accumulated.
     *
    ...
    /**
     * The respHist, or response histogram has a special structure: The lower
     * buckets are fine-grained buckets. The higher buckets are coarse-grained
     * bucket covering 10x as much time. It is done this way to save memory
     * (from 1000 entries per thread per operation, down to 370 entries). For
     * low response times, we care a lot about the exact response time and
     * therefore we use fine-grained buckets. For large response times we just
     * want to know the ballpark, but not the exact number. So it does not make
     * sense to keep the same bucket size throughout. We use fine granularity
     * below 1.5x largest set 90th% and coarse granularity for anything beyond
     * that.
     *
     * Now, we need to flatten the response time histogram into a flat one
     * before plotting. We do this, here. We'll end up with more entries, but we
     * really don't care since this is one copy, once per run at report time.
     */
    private void flattenRespHist() {
        ...
        if (limit > FINE_RESPBUCKETS) {
            ...
            for (int i = 0; i < txTypes; i++) {
                ...
                if (spareLastBucket) // Just copy the last bucket.
                {
